      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          <div className="inline-flex items-center space-x-2 bg-brand-gold/10 text-brand-gold px-4 py-1.5 rounded-full mb-4 font-sans text-xs font-bold uppercase tracking-widest">
            ...
            <span>{lang === "en" ? "Special Offers" : "Penawaran Spesial"}</span>
          </div>
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-serif font-light tracking-tight text-slate-950 mb-6">
            {lang === "en" ? (
              <>
                Most Favorite <span className="italic font-normal text-brand-gold">Tour Packages</span>
              </>
            ) : (
              <>
          ...
          <p className="text-slate-600 font-sans font-light text-sm sm:text-base md:text-lg leading-relaxed tracking-wide">
            {lang === "en"
              ? "Explore the beauty with fully curated, all-in-one travel packages integrating premium accommodation, comfortable transport, experienced guides, and exclusive entry tickets."
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10" id="packages-grid">
          {displayedPackages.map((pkg) => {
            const currentBadge = lang === "en" && pkg.enBadge ? pkg.enBadge : pkg.badge;
            const currentDestination = lang === "en" && pkg.enDestination ? pkg.enDestination : pkg.destination;
            const currentDuration = lang === "en" && pkg.enDuration ? pkg.enDuration : pkg.duration;
            const currentTitle = lang === "en" && pkg.enTitle ? pkg.enTitle : pkg.title;
            const currentDescription = lang === "en" && pkg.enDescription ? pkg.enDescription : pkg.description;
            const currentInclusions = lang === "en" && pkg.enInclusions ? pkg.enInclusions : pkg.inclusions;
              ...
              >
                ...
                <div className="p-7 flex flex-col flex-1">
                  ...
                  <div className="space-y-2.5 mb-8 flex-1">
                    <span className="block text-[9px] font-extrabold text-slate-400 tracking-wider uppercase font-sans mb-2">
                      {lang === "en" ? "Inclusions" : "Fasilitas Termasuk"}
                    </span>
                    {currentInclusions.slice(0, 4).map((inc, index) => (
                    ...
                    {currentInclusions.length > 4 && (
                      <span className="block text-[11px] font-sans text-slate-400 pl-6 italic font-medium">
                        {lang === "en"
                          ? `+ ${currentInclusions.length - 4} other exclusive services`
                          : `+ ${currentInclusions.length - 4} layanan eksklusif lainnya`
                        }
                      </span>
                    )}
                  </div>
                  ...
                  <div className="border-t border-slate-100 pt-6 mt-auto">
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <span className="block text-[9px] uppercase font-bold text-slate-400 tracking-wider font-sans mb-0.5">
                          {lang === "en" ? "All-In Rate" : "Tarif All-In"}
                        </span>
                      ...
                      >
                        {lang === "en" ? "Contact Agent" : "Hubungi Agen"}
                      </button>
                    </div>
                  </div>
